...
export default function Purchases() {
  ...
  const [hasPurchaseChanged, setHasPurchaseChanged] = useState(false);
  ...
  const remove = (id: string) => { setItems(items.filter((i: any) => i.productId !== id)); setHasChanges(true); setHasPurchaseChanged(true); };
  const updQty = (id: string, q: number) => { if (q <= 0) return remove(id); setItems(items.map((i: any) => i.productId === id ? { ...i, quantity: q, total: q * i.unitPrice } : i)); setHasChanges(true); setHasPurchaseChanged(true); };
  const updPrice = (id: string, v: string) => { const n = +v.replace(/,/g, '') || 0; setItems(items.map((i: any) => { if (i.productId !== id) return i; const pp = i.unit === 'كرتون' && i.boxQty ? n / i.boxQty : n; return { ...i, unitPrice: n, total: i.quantity * n, pricePerPiece: pp }; })); setHasChanges(true); setHasPurchaseChanged(true); };
  ...
  const openPf = (p?: any) => {
    if (p) {
      ...
      setPf({ name: p.name, barcode: p.barcode || '', quantity: '1', unit: p.unit || 'حبة', boxQty: p.boxQty?.toString() || '', unitPrice: cartonPrice.toString(), sellingPrice: (p.sellingPrice || '').toString(), minStock: (p.minStock || '20').toString() });
    } else {
  ...
  const handleBarcodeDetected = (bc: string) => {
    ...
    else { setPf({ name: '', barcode: bc, quantity: '1', unit: 'كرتون', boxQty: '', unitPrice: '', sellingPrice: '', minStock: '' }); setProdOpen(true); }
  };
  ...
  const reset = () => { setItems([]); setPaid(''); setEditPur(null); setOriginalPurchase(null); setHasPurchaseChanged(false); setInvoiceNo(''); setSupplier(''); setHasChanges(false); };

  const startEdit = (p: any) => {
    setEditPur(p);
    setOriginalPurchase({ ...p, items: p.items.map((i: any) => ({ ...i })) });
  ...
  const save = () => {
    ...
    if (editPur) {
      if (!hasPurchaseChanged) { alert('لا توجد تغييرات لحفظها.'); return; }
    ...
    <div className="page-container" ref={topRef}>
        ...
        <div className="space-y-1 mb-4">
          ...
          {items.map((i: any) => (
            <div key={i.productId} className="grid grid-cols-12 gap-2 items-center px-3 py-2 border-b border-border">
              <span className="col-span-4 font-semibold text-body truncate">{i.productName}{i.unit === 'كرتون' && i.boxQty ? ` (كرتون ${i.boxQty} ح)` : ''}</span>
              ...
              <div className="col-span-1 flex justify-end"><button onClick={() => { if (confirm('النقل لسلة المحذوفات؟')) remove(i.productId); }} className="p-1 text-danger"><Trash2 size={18} /></button></div>
            </div>
          ))}
        </div>
      )}
                ...
                <>
                  ...
                  <div className="absolute z-40 left-0 right-0 top-full mt-1 border border-border rounded-card bg-surface max-h-56 overflow-y-auto shadow-dialog">
                    {!filtered.length ? <p className="p-3 text-center text-small">لا توجد منتجات</p> : filtered.map((p: any) => (
                      <button key={p.id} onClick={() => openPf(p)} className="w-full flex items-center justify-between p-3 hover:bg-primary-light border-b border-border last:border-b-0">
                        <div className="text-right"><p className="font-semibold">{p.name}</p><p className="text-small text-text-secondary">{p.barcode || '-'}</p></div>
      ...
      {items.length > 0 && (
        <>
          <div className="flex justify-between items-start mb-3 gap-3">
            {!isCashPurchase && (
              <div className="flex-1">
                <Input label="المدفوع" type="text" inputMode="decimal" value={paid ? fmt(+paid) : ''} onChange={e => { setPaid(e.target.value.replace(/,/g, '')); setHasPurchaseChanged(true); }} />
                {+paid > 0 && remaining > 0 && <p className="text-small text-text-secondary mt-1">المتبقي: {fmt(remaining)} <span className="text-small">{currency}</span></p>}
              </div>
            )}
          ...
          {!isCashPurchase && <div className="mb-3"><Input label="رقم فاتورة المورد (اختياري)" value={invoiceNo} onChange={e => setInvoiceNo(e.target.value)} /></div>}
          <SaveButton onClick={save} isEditing={!!editPur} hasChanged={hasPurchaseChanged} labelUpdate="تحديث الفاتورة" icon={<ArrowDownToLine size={20} />} />
        </>
      )}

      <div className="mt-6">
        <h2 className="text-card-title mb-3">الفواتير السابقة</h2>
        {allPurchases.length === 0 ? <p className="text-small text-text-secondary text-center">لا توجد فواتير</p> : allPurchases.map((p: any) => (
          <div key={p.id}>
            <Card className="mb-3">
              <div className="flex justify-between items-start mb-2">
                <div>
                  <p className="text-small font-bold text-info mb-1">{supName(p.supplierId)}</p>
                  <span className="text-small font-semibold px-2 py-0.5 rounded bg-info/10 text-info">فاتورة شراء #{p.invoiceNo || p.id.replace(/\D/g, '').slice(-6)}</span>
                </div>
                <div className="flex gap-2">
            ...
            <div id={`invoice-${p.id}`} style={{ position: 'absolute', left: '-9999px', top: 0, width: '80mm', padding: '10px', fontFamily: 'Cairo, sans-serif', direction: 'rtl', backgroundColor: '#fff', color: '#000' }}>
              ...
              <table style={{ width: '100%', fontSize: '10px', borderCollapse: 'collapse' }}>
                <thead><tr style={{ borderBottom: '1px solid #000' }}><th style={{ textAlign: 'right', padding: '2px' }}>المنتج</th><th style={{ textAlign: 'center', padding: '2px' }}>الكمية</th><th style={{ textAlign: 'center', padding: '2px' }}>السعر</th><th style={{ textAlign: 'center', padding: '2px' }}>الإجمالي</th></tr></thead>
                <tbody>{p.items.map((i: any, idx: number) => (
                  <tr key={idx} style={{ borderBottom: '1px solid #ccc' }}><td style={{ textAlign: 'right', padding: '2px' }}>{i.productName}{i.unit === 'كرتون' && i.boxQty ? ` (${i.boxQty} ح)` : ''}</td><td style={{ textAlign: 'center', padding: '2px' }}>{i.quantity} {i.unit === 'كرتون' ? 'كرتون' : 'حبة'}</td><td style={{ textAlign: 'center', padding: '2px' }}>{fmt(i.unitPrice)}</td><td style={{ textAlign: 'center', padding: '2px' }}>{fmt(i.total)}</td></tr>))}</tbody>
              </table>
              <hr style={{ margin: '3px 0', border: 'none', borderTop: '1px dashed #000' }} />
      ...
      <BarcodeScanner open={barcodeOpen} onClose={() => setBarcodeOpen(false)} onDetected={handleBarcodeDetected} onProductFound={(product: any) => { openPf(product); }} />
    </div>
  );
}
